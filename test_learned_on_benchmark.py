...
parser = argparse.ArgumentParser(
    description='Arguments for test_learned_on_benchmark')
parser.add_argument('--Pn_j', type=int, default=30,
                    help='Number of jobs of instances to test')
parser.add_argument('--Pn_m', type=int, default=20,
...
parser.add_argument('--which_benchmark', type=str,
                    default='tai', help='Which benchmark to test')
...
path = './SavedNetwork/{}.pth'.format(str(N_JOBS_N) + '_' +
                                      str(N_MACHINES_N) + '_' + str(LOW) + '_' + str(HIGH))
...
dataLoaded = np.load('./BenchDataNmpy/' + benchmark +
                     str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.npy')
dataset = []
...
t2 = time.time()
file_writing_obj = open('./' + 'drltime_' + benchmark + '_' + str(N_JOBS_N) + 'x' +
                        str(N_MACHINES_N) + '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.txt', 'w')
file_writing_obj.write(str((t2 - t1)/len(dataset)))

# print(result)
# print(np.array(result, dtype=np.single).mean())
np.save('drlResult_' + benchmark + '_' + str(N_JOBS_N) + 'x' + str(N_MACHINES_N) +
        '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P), np.array(result, dtype=np.single))
